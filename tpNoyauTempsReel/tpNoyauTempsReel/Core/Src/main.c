...
void taskGive(void *unused)
{
	...
	for(;;)
	{
		...
		if (xQueueSend(timerQueue, &tickValue, portMAX_DELAY) == pdPASS)
		{
			printf("taskGive : envoi tick = %lu\r\n", tickValue);
		}
...
void taskTake(void *unused)
{
	...
	for(;;)
	{
		if (xQueueReceive(timerQueue, &receivedValue, portMAX_DELAY) == pdPASS)
		{
			printf("taskTake : recu tick = %lu\r\n", receivedValue);
		}
	}
}
...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
	//tasks (Créer toutes les tâches ici)

	printf("\r\n==== TP RTOS ====\r\n");
//	HAL_Delay(200);

	//binSem = xSemaphoreCreateBinary(); TaskHandle_t taskTakeHandle = NULL;
	//if (binSem == NULL)
	//{
